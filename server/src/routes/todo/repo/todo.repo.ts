...
export async function getTodoDataByIdRepo(id: number): Promise<TodoItem | null> {
    try {
        const data = await todoRepo.db.query<TodoItem>(`SELECT * from todocompany.todo AS td WHERE id=$1`, [id]);
...
export async function getTodoData(): Promise<TodoItem[] | null> {
    try {
        const data = await todoRepo.db.query<TodoItem>(`SELECT * from todocompany.todo order by created_at desc`);
        return data.rows;
    } catch (e) {
        return null;
    }
...
export async function createTodoDataRepo(todoItem: TodoItem): Promise<number | null> {
    try {
        const data = await todoRepo.db.query<{ id: number }>(`INSERT INTO todocompany.todo(description, name) VALUES ($1,$2) RETURNING id`, [
...
export async function updateTodoDataRepo<T = {}>(todoItem: TodoItem): Promise<number | null> {
    try {
        ...
        UPDATE todocompany.todo SET name=$1,description=$2 WHERE id=$3 RETURNING id
        `, [
            todoItem.name,
            todoItem.description,
            todoItem.id,
...
export async function deleteTodoDataRepo(id: number): Promise<boolean> {
    try {
        const res = await todoRepo.db.query<{id: number}>(`DELETE FROM todocompany.todo AS td WHERE id=$1 RETURNING id`, [id]);
